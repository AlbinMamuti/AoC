...
public:
    int x, y, z;
    Beacon(){};
    Beacon(int _x, int _y, int _z)
    {
        x = _x;
        y = _y;
        z = _z;
    }
    string offsetHash(Beacon b)
    {
        stringstream s;
        s << "[" << b.x - x << "," << b.y - y << "," << b.z - z << "]";
        return s.str();
    }
    int offset(Beacon b) { return std::abs(b.x - x) + std::abs(b.y - y) + std::abs(b.z - z); }
...
int main(int argc, char *argv[])
{
    ...
    while (!scanners.empty())
    {
        ...
        for (auto m : matching)
        {
            ...
            if (!dupe)
                candidates.push_back(candidate); // these are beacons not already in grid
        }
